__d("GraphQLStoreWriter",["DliteLog","GraphQL_EXPERIMENTAL","GraphQLConstants","GraphQLMutatorConstants","GraphQLRange","GraphQLRangeTracker","GraphQLStoreDataHandler","GraphQLStoreQueryPath","GraphQLStoreRangeUtils","generateClientID","generateClientEdgeID","invariant","isEmpty"],function (global/*e*/, require/*t*/, requireDynamic/*n*/, requireLazy/*r*/, module/*i*/) {
    ...
    function s(global/*e*/, require/*t*/) {
        if (1 === global/*e*/.length && global/*e*/[0] === D) return GraphQLConstants/*p*/.VIEWER_CLIENT_ID;
    ...
    var DliteLog/*u*/ = require/*t*/("DliteLog"),
        GraphQL_EXPERIMENTAL/*c*/ = require/*t*/("GraphQL_EXPERIMENTAL"),
        GraphQLConstants/*p*/ = require/*t*/("GraphQLConstants"),
        GraphQLMutatorConstants/*d*/ = require/*t*/("GraphQLMutatorConstants"),
        GraphQLRange/*h*/ = require/*t*/("GraphQLRange"),
        GraphQLRangeTracker/*f*/ = require/*t*/("GraphQLRangeTracker"),
        GraphQLStoreDataHandler/*m*/ = require/*t*/("GraphQLStoreDataHandler"),
        GraphQLStoreQueryPath/*g*/ = require/*t*/("GraphQLStoreQueryPath"),
        GraphQLStoreRangeUtils/*_*/ = require/*t*/("GraphQLStoreRangeUtils"),
        generateClientID/*y*/ = require/*t*/("generateClientID"),
        generateClientEdgeID/*v*/ = require/*t*/("generateClientEdgeID"),
        invariant/*S*/ = require/*t*/("invariant"),
        isEmpty/*b*/ = require/*t*/("isEmpty"),
    ...
    o.prototype.handleFieldMergeMutation = function(global/*e*/, require/*t*/) {
        ...
        1 !== requireDynamic/*n*/.length && invariant/*S*/(0, "Simple mutations must have exactly one top-level payload field.");
        var requireLazy/*r*/, module/*i*/ = requireDynamic/*n*/[0];
        module/*i*/ === D && (requireLazy/*r*/ = GraphQLStoreQueryPath/*g*/.prunePathForDataID(GraphQLConstants/*p*/.VIEWER_CLIENT_ID));
        ...
        for (var requireDynamic/*n*/ in global/*e*/)
            if (global/*e*/.hasOwnProperty(requireDynamic/*n*/)) {
                var requireLazy/*r*/ = global/*e*/[requireDynamic/*n*/];
                if ("object" == typeof requireLazy/*r*/ && ("id" in requireLazy/*r*/ || requireDynamic/*n*/ === GraphQLConstants/*p*/.VIEWER || Array.isArray(requireLazy/*r*/))) {
                    var module/*i*/ = {};
        ...
        module/*i*/.node || invariant/*S*/(0, "newly added edge must have node field");
        var o;
        requireDynamic/*n*/.sourceID ? o = requireDynamic/*n*/.sourceID : (module/*i*/.source.id || invariant/*S*/(0, "add mutation without sourceID in the config must have an id in the source field of the new edge"), o = module/*i*/.source.id);
        var a = global/*e*/.client_mutation_id,
            s = this.$GraphQLStoreWriter_getRanges(o, requireDynamic/*n*/.connectionName),
            DliteLog/*u*/ = module/*i*/.node.id ? module/*i*/.node.id : generateClientID/*y*/();
        module/*i*/.node.__dataID__ = DliteLog/*u*/, this.$GraphQLStoreWriter_isOptimisticUpdate ? module/*i*/.node.id = DliteLog/*u*/ : this.$GraphQLStoreWriter_shouldUpdateTrackedQueries = !0, module/*i*/.cursor = module/*i*/.cursor || GraphQLConstants/*p*/.DUMMY_CURSOR_ID;
        for (var GraphQL_EXPERIMENTAL/*c*/ = 0; GraphQL_EXPERIMENTAL/*c*/ < s.length; GraphQL_EXPERIMENTAL/*c*/++) {
            var GraphQLRange/*h*/ = l(requireLazy/*r*/, s[GraphQL_EXPERIMENTAL/*c*/].calls.toString());
            if (GraphQLRange/*h*/) {
                ...
                var GraphQLStoreRangeUtils/*_*/ = generateClientEdgeID/*v*/(GraphQLRangeTracker/*f*/, DliteLog/*u*/),
                    ...
                    R = function(global/*e*/) {
                        ...
                        require/*t*/ && (this.storeNodeWithID(require/*t*/, module/*i*/, GraphQLStoreRangeUtils/*_*/, GraphQLStoreQueryPath/*g*/), isEmpty/*b*/ = !0), global/*e*/.getFragments().forEach(R)
                    }.bind(this);
                if (R(require/*t*/), isEmpty/*b*/ || invariant/*S*/(0, "Mutation query must include the relevant edge"), this.$GraphQLStoreWriter_isOptimisticUpdate) this.$GraphQLStoreWriter_handleOptimisticRangeUpdate(this.$GraphQLStoreWriter_queuedData[GraphQLStoreRangeUtils/*_*/], GraphQLRangeTracker/*f*/, GraphQLRange/*h*/);
                else {
        ...
        void 0 === requireDynamic/*n*/ && invariant/*S*/(0, "deleted id must be in the payload");
        ...
        var requireDynamic/*n*/ = global/*e*/[require/*t*/.deletedIDFieldName];
        void 0 === requireDynamic/*n*/ && invariant/*S*/(0, "deleted id must be in the payload"), this.deleteNodeWithID(requireDynamic/*n*/)
    }, o.prototype.deleteNodeFromRangeWithID = function(global/*e*/, require/*t*/) {
        var requireDynamic/*n*/ = generateClientEdgeID/*v*/(require/*t*/, global/*e*/);
        if (this.$GraphQLStoreWriter_isOptimisticUpdate) this.$GraphQLStoreWriter_handleOptimisticRangeUpdate(requireDynamic/*n*/, require/*t*/, GraphQLMutatorConstants/*d*/.REMOVE);
        else {
        ...
        requireLazy/*r*/ = GraphQLStoreQueryPath/*g*/.prunePathForDataID(requireDynamic/*n*/, requireLazy/*r*/);
        var module/*i*/ = this.$GraphQLStoreWriter_getNodeSink();
        module/*i*/[requireDynamic/*n*/] || (module/*i*/[requireDynamic/*n*/] = GraphQLStoreDataHandler/*m*/.createPointerWithID(requireDynamic/*n*/), this.$GraphQLStoreWriter_shouldUpdateTrackedQueries || (this.$GraphQLStoreWriter_newlyAddedIDs[requireDynamic/*n*/] = !0)), GraphQLStoreDataHandler/*m*/.isClientID(requireDynamic/*n*/) && !module/*i*/[requireDynamic/*n*/].__path__ && (module/*i*/[requireDynamic/*n*/].__path__ = requireLazy/*r*/.clone()), this.$GraphQLStoreWriter_traverseNode(global/*e*/, require/*t*/, module/*i*/[requireDynamic/*n*/], requireLazy/*r*/), (this.$GraphQLStoreWriter_shouldUpdateTrackedQueries || this.$GraphQLStoreWriter_newlyAddedIDs[requireDynamic/*n*/]) && this.$GraphQLStoreWriter_queryTracker.addQueryForID(requireDynamic/*n*/, global/*e*/)
        ...
        for (var l = global/*e*/.getFragments(), DliteLog/*u*/ = 0; DliteLog/*u*/ < l.length; DliteLog/*u*/++) this.$GraphQLStoreWriter_traverseNode(l[DliteLog/*u*/], require/*t*/, requireDynamic/*n*/, requireLazy/*r*/, module/*i*/ || global/*e*/)
        ...
        var o, a, s = requireLazy/*r*/.__dataID__,
            l = GraphQLStoreRangeUtils/*_*/.getStorageKeyForField(require/*t*/, {}),
            DliteLog/*u*/ = require/*t*/.getGeneratedAlias();
        if (o = requireDynamic/*n*/.hasOwnProperty(DliteLog/*u*/) ? requireDynamic/*n*/[DliteLog/*u*/] : requireDynamic/*n*/[require/*t*/.getGeneratedFieldName()], Array.isArray(o))
            if (l == R && GraphQLStoreRangeUtils/*_*/.isRangeNode(global/*e*/)) {
                var GraphQL_EXPERIMENTAL/*c*/ = global/*e*/.getCalls();
                if (isEmpty/*b*/(GraphQL_EXPERIMENTAL/*c*/)) return void console.error("The field %s is a range node, and thus expected to have a first() or last() filter call", global/*e*/.getFieldName());
                s || invariant/*S*/(0, "Parent of edges must have an ID");
                for (var GraphQLMutatorConstants/*d*/, GraphQLStoreQueryPath/*g*/ = require/*t*/.getEdgesID() && GraphQLRangeTracker/*f*/.isFieldTracked(require/*t*/, s), T = 0; T < o.length; T++) {
                    var x = o[T],
                        P = x.node;
                    if (null === P) GraphQLMutatorConstants/*d*/ = GraphQLMutatorConstants/*d*/ || o.slice(0, T);
                        ...
                        P.__dataID__ = I, x.__dataID__ = F, this.$GraphQLStoreWriter_nodeRangeTracker.addRangeIDForNodeID(s, I), this.storeNodeWithID(require/*t*/, x, F, module/*i*/), GraphQLMutatorConstants/*d*/ && GraphQLMutatorConstants/*d*/.push(x), GraphQLStoreQueryPath/*g*/ && GraphQLRangeTracker/*f*/.trackEdge(require/*t*/, F, s)
                    }
                }
                ...
                !M.__range__ || Q ? (requireLazy/*r*/.__range__ = new GraphQLRange/*h*/(this.$GraphQLStoreWriter_nodeData), requireLazy/*r*/.__filterCalls__ = GraphQLStoreRangeUtils/*_*/.getFilterCalls(GraphQL_EXPERIMENTAL/*c*/), L = !0, Q && (requireLazy/*r*/.__forceIndex__ = this.$GraphQLStoreWriter_forceIndex)) : requireLazy/*r*/.__range__ = M.__range__;
                var A = requireDynamic/*n*/[E];
                A || (A = {
                    has_previous_page: !1,
                    has_next_page: !1
                }), requireLazy/*r*/.__range__.addItems(GraphQL_EXPERIMENTAL/*c*/, {}, o, A), isEmpty/*b*/(M) || isEmpty/*b*/(o) || this.$GraphQLStoreWriter_notifyOnChange(s)
            } else {
                ...
                for (var O = 0; O < o.length; O++)
                    if ("object" != typeof o[O]) requireLazy/*r*/[l][O] !== o[O] && (requireLazy/*r*/[l][O] = o[O], k || (this.$GraphQLStoreWriter_notifyOnChange(s), k = !0));
                        ...
                        a || (a = generateClientID/*y*/()), requireLazy/*r*/[l][O] && requireLazy/*r*/[l][O].__dataID__ === a || (requireLazy/*r*/[l][O] = GraphQLStoreDataHandler/*m*/.createPointerWithID(a), k || (this.$GraphQLStoreWriter_notifyOnChange(s), k = !0)), this.storeNodeWithID(require/*t*/, o[O], a, module/*i*/)
                    }
            ...
            l === D ? a = GraphQLConstants/*p*/.VIEWER_CLIENT_ID : l == w ? a = o.__dataID__ || o.id || generateClientID/*y*/() : (a = o.id, !a && s && (N = this.$GraphQLStoreWriter_nodeData[s], N && N[l] ? a = N[l].__dataID__ : requireLazy/*r*/ && requireLazy/*r*/[l] && (a = requireLazy/*r*/[l].__dataID__)), a || (a = generateClientID/*y*/())), this.$GraphQLStoreWriter_writePointer(requireLazy/*r*/, l, a), this.storeNodeWithID(require/*t*/, o, a, module/*i*/)
        ...
        requireDynamic/*n*/ in GraphQLMutatorConstants/*d*/.RANGE_OPERATIONS || invariant/*S*/(0, "Invalid operation %s provided", requireDynamic/*n*/), requireDynamic/*n*/ === GraphQLMutatorConstants/*d*/.REMOVE && "string" == typeof global/*e*/ || requireDynamic/*n*/ !== GraphQLMutatorConstants/*d*/.REMOVE && "object" == typeof global/*e*/ || invariant/*S*/(0, "Incorrect edge information provided");
        var requireLazy/*r*/ = this.$GraphQLStoreWriter_queuedData;
        require/*t*/ in requireLazy/*r*/ || (requireLazy/*r*/[require/*t*/] = {}), requireDynamic/*n*/ in requireLazy/*r*/[require/*t*/] || (requireLazy/*r*/[require/*t*/][requireDynamic/*n*/] = []), requireDynamic/*n*/ === GraphQLMutatorConstants/*d*/.PREPEND ? requireLazy/*r*/[require/*t*/][requireDynamic/*n*/].unshift(global/*e*/) : requireLazy/*r*/[require/*t*/][requireDynamic/*n*/].push(global/*e*/)
        ...
        if (requireLazy/*r*/)
            for (var o in requireLazy/*r*/) {
                ...
                if (s === require/*t*/) {
                    ...
                    l && l.__range__ && l.__range__ instanceof GraphQLRange/*h*/ && l.__filterCalls__ ? module/*i*/.push({
        ...
        global/*e*/[require/*t*/] && requireDynamic/*n*/ === global/*e*/[require/*t*/].__dataID__ || (global/*e*/[require/*t*/] = GraphQLStoreDataHandler/*m*/.createPointerWithID(requireDynamic/*n*/), this.$GraphQLStoreWriter_notifyOnChange(global/*e*/.__dataID__))
